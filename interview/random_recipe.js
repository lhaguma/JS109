...
function random(array, n) {
  ...
  while (n > 0 && elements.length > 0) {
    let randomIndex = Math.floor(Math.random() * elements.length); //randome index
    let randomElement = elements[randomIndex]; //get random element from random index

    randomElements.push(randomElement); //put random element into array
    elements.splice(randomIndex, 1); //remove the random element from array
    n--;
  }
